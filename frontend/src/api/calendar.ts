...
export async function fetchCalendarWindow(token: string, centerDate: Date): Promise<CalendarWindowData> {
  ...
  returnedMonths.forEach((monthKey, index) => {
    const matchingDate = emptyWindow.monthDates.find(date => getCalendarMonthKey(date).toUpperCase() === monthKey.toUpperCase())
...
export async function saveCalendarWindow(token: string, calendarWindow: CalendarWindowData): Promise<void> {
