...
export default function Recipes() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // If navigated from Calendar or Tournament, open that recipe
      const pendingId = sessionStorage.getItem('viewRecipeId');
      if (pendingId) {
        sessionStorage.removeItem('viewRecipeId');
        const target = data.find((r) => String(r.id) === pendingId);
        if (target) {
          setSelected(target);
          setCurrentStep(0);
