...
export const getCalendar = async (req: Request, res: Response) => {
  try {
    ...
    try {
      requestedMonths = JSON.parse(String(months)).map((m: string) => m.toUpperCase());
      if (!Array.isArray(requestedMonths)) throw new Error();
    } catch {
      res.status(400).json({ message: 'months must be a JSON array of month:year strings' });
      return;
    }

    const existing = db.select().from(calendar).where(eq(calendar.userId, userId)).get();
...
export const setCalendar = async (req: Request, res: Response) => {
  try {
    ...
    if (!months || !days) {
      res.status(400).json({ message: 'months and days properties are required' });
      return;
    }
    const monthsJson = JSON.stringify(months.map((m: string) => m.toUpperCase()));
